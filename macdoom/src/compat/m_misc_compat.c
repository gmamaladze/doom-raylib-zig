...
//	PCX Screenshots.
//
// NOTE: This is a patched copy of linuxdoom-1.10/m_misc.c for 64-bit
// compatibility. The original uses (int) to store both integer values
// and string pointers in the default_t struct, which breaks on 64-bit.
// Changes: int defaultvalue -> intptr_t defaultvalue, casts updated.
//
//-----------------------------------------------------------------------------
...
// 64-bit fix: use intptr_t for defaultvalue to hold both ints and pointers
typedef struct
{
    char*	name;
    int*	location;
